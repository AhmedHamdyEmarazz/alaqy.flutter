...
class _NewMessagesState extends State<NewMessages> {
  ...
  void sendpushnotificationx(String token, String body, String title) async {
    @pragma('vm:entry-point')
    Future<void> _onBackgroundMessage(RemoteMessage message) async {
      await Firebase.initializeApp(
          // options: DefaultFirebaseOptions.currentPlatform,
  ...
  Widget build(BuildContext context) {
    ...
    void foundsentseen() async {
      final useri = await FirebaseFirestore.instance
          ...
          .get();
      final idx = useri.docs.isNotEmpty ? useri.docs.first.id : 'i';
      final sent =
          useri.docs.isNotEmpty ? useri.docs.first.data()['foundsent'] : 'i';
      final seen =
          useri.docs.isNotEmpty ? useri.docs.first.data()['foundseen'] : 'i';

      final upnum =
          useri.docs.isNotEmpty ? useri.docs.first.data()['upnum'] : 'i';
      final upnumseen =
          useri.docs.isNotEmpty ? useri.docs.first.data()['upnumseen'] : 'i';

      final storenameb = useri.docs.isNotEmpty
          ? useri.docs.first.data()['basicstore_name']
          : 'i';
      final listcont =
      ...
      final mustaddedseen = foundseenx.fold(
          0, (previousValue, element) => previousValue + element);
      final fix = await FirebaseFirestore.instance
          .collection('business_details')
          .doc(idx)
          .get();
      fix.exists
    ...
    void _sendMessagecustomer() async {
      print('خخخخخخخخخخخخخخخخخخخخخخخخخخخخخخخخخ');
      final chatxx = await FirebaseFirestore.instance
          .collection('chat')
          .doc(widget.id.toString())
          .collection('chatx')
...
// 'customerid2':userx,
      ...
      print(tokenx);
      print(",,,,,,,,<<<<>>>>>>>>>");
      // FirebaseFirestore.instance.collection('notify').doc().set({
      //   'customerid': cusid,
      //   'businessid': {widget.busid},
      //   'listid': widget.id,
      //   'createdAt': Timestamp.now(),
      ...
      // });
      //   sendpushnotificationx(tokenx, _enteredMessage, widget.username);
      final business = await FirebaseFirestore.instance
          .collection('business_details')
          .doc(widget.busid.toString())
          .get();
      final foundsent = business.exists ? business.data()!['foundsent'] : '';
      business.exists
          ? FirebaseFirestore.instance
              .collection('business_details')
              .doc(widget.busid.toString())
      ...
      final del = await FirebaseFirestore.instance
          ...
          .get();
      final dele = del.docs.last;
      final delete = dele.id;
      final delet = dele.data()['text'];
      final delex = dele.data()['to'];
      final idx = del.docs.lastWhere((element) =>
          element.data()['text'].toString().endsWith('permission'));
      print(idx.id);
      print(',,,,,,,,,,,,,,,,,``````````````,,,,,,,,,,,,');

      delex != FirebaseAuth.instance.currentUser!.uid && idx != -1
          ? FirebaseFirestore.instance
              .collection('chat')
              .doc(widget.id.toString())
              .collection('chatx')
    ...
    void _sendMessage() async {
      print('عااااااااعااااااااااااعععععععععععاااا');
      FirebaseMessaging messaging = FirebaseMessaging.instance;
      final chatxx = await FirebaseFirestore.instance
          .collection('chat')
          .doc(widget.id.toString())
      ...
      if (widget.type == 'business') {
        ...
        chatxx.exists
            ? await FirebaseFirestore.instance
                ...
                .update({'combo': 'xxx'})
            : null;
        final chatxxx = await FirebaseFirestore.instance
            .collection('chat')
            .doc(widget.id.toString())
            .collection('chatx')
            .doc('${widget.basicusername}busloading${widget.basicstorename}')
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                        labelText: block == 'unchain' && blovsunblo == 'block'
                            ? 'ارسال نص...'
                            : blovsunblo == 'unblock'
                                ? 'لقد قمت بحظر هذا الحساب ..الغ الحظر لاكمال المحادثة ...'
                                : 'للاسف لقد تم حظرك من قبل هذا المستخدم'),
                    onChanged: (value) {
                      setState(() {
...
//                       color: Colors.cyan.shade100,
                      ...
                      dropdownColor: Color.fromARGB(39, 255, 193, 7),
                          ...
                          repan == 'deactivated'
                              ...
                              : showDialog(
                                  ...
                                  builder: (ctx) => AlertDialog(
                                          title: const Text(
                                              'من فضلك ضع اسبابك للإبلاغ عن هذه المحادثة'),
                                                ...
                                                if (reportcontroller
                                                  ...
                                                  showDialog(
                                                    ...
                                                    builder: (ctx) =>
                                                      ...
                                                      title: const Text(
                                                          'تم ارسال البلاغ !'),
                                                      content: const Text(
                                                        '.. شكرا ،،سيتم التقييم من قبل المسئولين',
                                                      ),
                                                      actions: <Widget>[
                                                        TextButton(
                                                          child: const Text(
                                                  ...
                                                  FirebaseFirestore.instance
                                                      ...
                                                      .doc(bancheck1
                                                          .data()!['reported'])
                                                      .set({
                                                    'chat/${Timestamp.now().toString()}':
                                                        Timestamp.now(),
                                                    'user':
                                                        'removetoban/${bancheck1.data()!['reported']}',
                                                  });
                                                  //_report();
                                                }
                                              },
